...
clean_up () {
    rm -rf openwrt*.img* ${img_path}/openwrt*.img* sha256sums-gh* *update.sh*
}
#硬盘检查
hd_check () {
    hd_id='mmcblk0'
...
repo_set () {
    ...
    firmware_id=openwrt-x86-64-generic-squashfs-combined-efi-gh.img
}
#寻找固件
search_file () {
    cd ${work_path} && clean_up && days=$(($days+1))
    #echo `${repo_url}/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean/sha256sums-gh`
    wget -q ${repo_url}/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean/sha256sums-gh
    exist_judge
}
#存在判断
exist_judge () {
    if [ -f sha256sums-gh ]; then
...
firmware_check () {
    if [ -f ${img_path}/${firmware_id}  ]; then
        ...
        echo -e '\e[92m对比下列sha256sum值，检查固件是否完整\e[0m' && grep -i ${firmware_id}.gz sha256sums-gh
    else
